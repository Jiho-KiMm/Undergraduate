...
class Enemy(Tank):
	...
	def __init__(self, level, type, position = None, direction = None, filename = None):
		...
		global enemies, sprites, stage_round
		...
		if self.type == self.TYPE_BASIC:
			self.speed = 1 + stage_round
		elif self.type == self.TYPE_FAST:
			self.speed = 3 + stage_round
		elif self.type == self.TYPE_POWER:
			self.superpowers = 1 + stage_round
		elif self.type == self.TYPE_ARMOR:
			self.health = 400 + stage_round*100
			# 수정한 부분
			self.speed = 1 + stage_round
...
class Game():
	...
	def __init__(self):
		...
		global screen, sprites, play_sounds, sounds, stage_round, bonus_game_stage

		# center window
		os.environ['SDL_VIDEO_WINDOW_POS'] = 'center'
		
		pygame.init()
		self.BonusClearSound = pygame.mixer.Sound("sounds/StageClear.mp3")
		self.FailedBonusSound = pygame.mixer.Sound("sounds/FailedStage.mp3")
		...
		self.clock = pygame.time.Clock()  #프레임 수 맞추기
		...
		self.bouns_stage = 1
		self.End_bonus_stage = False

		self.Clear_bonus_stage = True

		self.bonus_game_stage = BonusGameStage()
	...
	def showMenu(self):
		...
		main_loop = True ################################################################### main loop
		while main_loop:
			time_passed = self.clock.tick(50)

			for event in pygame.event.get():  # pygame main loop
				if event.type == pygame.QUIT:  # 프로그램 닫기
					quit()
				elif event.type == pygame.KEYDOWN:
					if event.key == pygame.K_q: # 프로그램 닫기
						quit()
					elif event.key == pygame.K_UP:
						if self.nr_of_players == 2: # 플레이어 2명
							self.nr_of_players = 1  # 플레이어 1명
	...
	def draw(self):
		...
		if self.game_over:													# 플레이어 2명일 때 수정
		...
		if pygame.font.get_init():
			...
			for n in range(len(players)):
				if n == 0:
					...
					screen.blit(self.font.render("POW", False, text_color), [x+14, y+350-15])
					screen.blit(self.font.render(str(n+1)+"P", False, text_color), [x+16, y+350])
					screen.blit(self.font.render(str(players[n].superpowers), False, text_color), [x+31, y+365])  #수정
					...
					screen.blit(self.font.render(str(n+1)+"P", False, text_color), [x+16, y+365+15])
					screen.blit(self.font.render(str(players[n].superpowers), False, text_color), [x+31, y+365+30])  #수정
	...
	def animateIntroScreen(self):
		...
		screen.fill([0, 0, 0]) #전체를 투명한 검정색으로 지운다
	...
	def nextLevel(self):
		...
		global castle, players, bullets, bonuses, play_sounds, sounds, stage_round
		...
		if self.Clear_bonus_stage == True:
			for player in players:
				player.lives += 2
				player.superpowers += 1
			self.Clear_bonus_stage = False

		# load level
		self.stage += 1

		# 이 부분 수정
		if self.End_bonus_stage == False:
			self.ChangeState()
		elif self.End_bonus_stage == True:
			screen.fill([0, 0, 0])
			self.startgame = pygame.time.get_ticks()

			while True:
				self.endgame = pygame.time.get_ticks()
				screen.blit(self.font.render("Stage" + str(self.stage), True, pygame.Color('white')), [195, 195])
				pygame.display.flip()

				if (self.endgame - self.startgame)/1000 > 2:
					break				
			self.End_bonus_stage = False
		...
		if stage_round > 0:
			while 1:
				for _ in range(4):
					self.EnemyList.append(self.rd.randint(0, 12))
				if sum(self.EnemyList) == 12:
					break
				else :
		...
		if stage_round == 0:
			levels_enemies = (
			(20,0,0,0), (14,4,0,2), (14,4,0,2), (2,5,10,3), (8,5,5,2),
			...
			enemies_l = [2 + self.EnemyList[0],2+ self.EnemyList[1],2+ self.EnemyList[2],2+ self.EnemyList[3]]
	...
	# 여기부터 따로 짠 코드 입니다!! ----------------------------------------------------------------------------------------------------------------------
	# ChangeState --> Stage를 바꿀 수 있도록 만든 함수 부분입니다.
			...
			for event in pygame.event.get():
				if event.type == pygame.KEYDOWN:
					...
					if event.key == pygame.K_RETURN:
						...
						if self.stage == 1 and stage_round >= 1:
							stage_round -= 1
							self.stage = 35
						elif self.stage <= 35:
							if (self.stage == 1):
	...
	# 어떤 보너스게임에 도전할지 정해지는 함수입니다.
	def BonusGame(self):
		global screen, sprites, stage_round, bonus_game_stage, players, CheckBonusGame, total_score, QuizFlag
		
		self.starttime = time.time()

		self.tomp = self.stage 
		...
		while 1:
			...
			screen.blit(self.font.render("Bonus Stage " + str(self.bouns_stage), True, pygame.Color('white')), [130, 180])

			pygame.display.flip()
			if self.endtime > self.starttime + self.elapsed_time:
				break

		screen.fill([0, 0, 0])
		pygame.display.flip()

		self.stage = self.tomp
		self.EnterPressKey = True

		# 각 게임의 설명이 나오고 보너스 게임을 실행하도록 했습니다.
		if self.bouns_stage == 1 or self.stage == 10:
			if self.stage == 10:
				while self.EnterPressKey:
					screen.fill([0, 0, 0])
					screen.blit(self.font1.render("How to play the game", True, pygame.Color('white')), [1, 1])
					screen.blit(self.font2.render("Simply avoid the enemy tanks coming from", True, pygame.Color('white')), [1, 41])
					screen.blit(self.font2.render("the right by using the arrow keys.", True, pygame.Color('white')), [1, 61])
					screen.blit(self.font2.render("If you collide with any obstacles, the game ends", True, pygame.Color('white')), [1, 91])
					screen.blit(self.font2.render("and you won't receive any rewards.", True, pygame.Color('white')), [1, 111])
					screen.blit(self.font2.render("Additionally, if you manage to survive until ", True, pygame.Color('white')), [1, 141])
					screen.blit(self.font2.render("the timer in the top left corner runs out,", True, pygame.Color('white')), [1, 161])
					screen.blit(self.font2.render("the game concludes, and you'll receive rewards.", True, pygame.Color('white')), [1, 181])
					screen.blit(self.font1.render("Please press the Enter", True, pygame.Color('white')), [1, 300])
					pygame.display.flip()
					for event in pygame.event.get():
						if event.type == pygame.KEYDOWN:
							screen.fill([0, 0, 0])
							if event.key == pygame.K_RETURN:
								self.EnterPressKey = False
				self.bonus_game_stage.MiniGame1()
				if CheckBonusGame == True:
					self.ClearBonus()
					self.Clear_bonus_stage = True
					...
					self.FailedBonus()
				self.bouns_stage += 1
				self.End_bonus_stage = True
		elif self.bouns_stage == 2 or self.stage == 20:
			if self.stage == 20:
				while self.EnterPressKey:
					screen.fill([0, 0, 0])
					screen.blit(self.font1.render("How to play the game", True, pygame.Color('white')), [1, 1])
					screen.blit(self.font2.render("This is a bonus game where you solve quizzes.", True, pygame.Color('white')), [1, 41])
					screen.blit(self.font2.render("A total of 5 quizzes will be presented, ", True, pygame.Color('white')), [1, 71])
					screen.blit(self.font2.render("and you'll select answers using", True, pygame.Color('white')), [1, 91])
					screen.blit(self.font2.render("the left and right arrow keys.", True, pygame.Color('white')), [1, 111])
					screen.blit(self.font2.render("If you don't make a selection before the timer", True, pygame.Color('white')), [1, 141])
					screen.blit(self.font2.render("in the bottom left corner runs out,", True, pygame.Color('white')), [1, 161])
					screen.blit(self.font2.render("the answer will be considered incorrect.", True, pygame.Color('white')), [1, 181])
					screen.blit(self.font2.render("If you correctly answer a total of 3 quizzes,", True, pygame.Color('white')), [1, 211])
					screen.blit(self.font2.render("you'll receive a game bonus", True, pygame.Color('white')), [1, 231])
					screen.blit(self.font2.render("otherwise, you won't receive the game bonus.", True, pygame.Color('white')), [1, 251])
					screen.blit(self.font1.render("Please press the Enter", True, pygame.Color('white')), [1, 300])
					pygame.display.flip()
					for event in pygame.event.get():
						if event.type == pygame.KEYDOWN:
							...
							if event.key == pygame.K_RETURN:
								self.EnterPressKey = False				
				self.bonus_game_stage.MiniGame2()
				if CheckBonusGame == True:
					self.ClearBonus()
					self.Clear_bonus_stage = True
					CheckBonusGame = False
				...
				self.End_bonus_stage = True
				QuizFlag = False
		elif self.bouns_stage == 3 or self.stage == 30:
			if self.stage == 30:
				while self.EnterPressKey:
					screen.fill([0, 0, 0])
					screen.blit(self.font1.render("How to play the game", True, pygame.Color('white')), [1, 1])
					screen.blit(self.font2.render("This is a bonus game where you navigate a maze", True, pygame.Color('white')), [1, 41])
					screen.blit(self.font2.render("towards the enemy tank in the bottom right corner. ", True, pygame.Color('white')), [1, 61])
					screen.blit(self.font2.render("The maze alternates between being ", True, pygame.Color('white')), [1, 91])
					screen.blit(self.font2.render("visible and invisible over time, ", True, pygame.Color('white')), [1, 111])
					screen.blit(self.font2.render("so you need to remember it well to escape.", True, pygame.Color('white')), [1, 131])
					screen.blit(self.font2.render("If you reach within the in-game timer,", True, pygame.Color('white')), [1, 161])
					screen.blit(self.font2.render("you can earn a bonus reward", True, pygame.Color('white')), [1, 181])
					screen.blit(self.font2.render("otherwise, you won't receive the bonus.", True, pygame.Color('white')), [1, 201])
					screen.blit(self.font1.render("Please press the Enter", True, pygame.Color('white')), [1, 300])
					pygame.display.flip()
					for event in pygame.event.get():
						if event.type == pygame.KEYDOWN:
							...
							if event.key == pygame.K_RETURN:
								self.EnterPressKey = False					
				self.bonus_game_stage.MiniGame3()
				if CheckBonusGame == True:
					self.ClearBonus()
					self.Clear_bonus_stage = True
					CheckBonusGame = False
	...
	def ClearBonus(self):
		...
		screen.fill([0, 0, 0])
		pygame.display.flip()
		self.BonusClearSound.play()

		while 1:
			self.endtime = time.time()
			screen.blit(self.font.render("Clear Bonus Stage", True, pygame.Color('white')), [100, 180])
	...
	#보너스 게임을 실패 했을 때 함수입니다
	def FailedBonus(self):

		self.starttime = time.time()

		screen.fill([0, 0, 0])
		pygame.display.flip()
		self.FailedBonusSound.play()

		while 1:
			self.endtime = time.time()
			screen.blit(self.font.render("Failed Bonus Stage", True, pygame.Color('white')), [100, 180])
...
# 보너스 게임의 구체적인 내용의 클래스입니다
class BonusGameStage():
	def __init__(self):

		global screen, sprites, stage_round, back_ground_sound

		# self.stage_N = stage_number
		...
		sprites = pygame.transform.scale(pygame.image.load("images/sprites.gif"), [192, 224])
		self.player = sprites.subsurface(0*2, 0*2, 13*2, 13*2)
	

		self.player_size = self.player.get_rect().size
		self.player_width = self.player_size[0]
		self.plyaer_height = self.player_size[1]
		self.player_x_pos = (480 / 2) - (self.player_width / 2)
		self.player_y_pos = (416 / 2) - (self.plyaer_height / 2)
		...
		self.dumy = sprites.subsurface(16*10, 0*2, 16*2, 16*2)
		self.dir = 0
		self.prev_dir = 0

		self.brick_image = sprites.subsurface(48*2, 64*2, 8*2, 8*2)
	# 3번째 미니게임 함수입니다
	def MiniGame3(self):

		global screen, sprites, stage_round, CheckBonusGame, GameStart, flag_, back_ground_sound

		self.player_init_x = 0
		self.player_init_y = 0
		self.player_speed = 5
		self.player_image = pygame.transform.rotate(self.player, 180)
		self.water_image = pygame.transform.rotate(self.dumy, 90)
		self.player_x_pos = 0
		self.player_y_pos = 0
		...
		self.dir = 0
		self.collide = -1
		screen.blit(self.player_image, (self.player_init_x, self.player_init_y))
		pygame.display.flip()
		...
		self.maze_map = 38
		# self.maze_map = random.randint(36, 38)
		self.lv = Level(self.maze_map)
		self.rects = self.lv.obstacle_rects

		self.elapsedtime = 40
		self.startgame = pygame.time.get_ticks()
		# .collidelist(self.level.obstacle_rects) != -1:
		...
		while GameStart:
			...
			self.collide = self.player_rect.collidelist(self.rects)
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYDOWN:
					if event.key == pygame.K_UP:
						self.Flag[0] = True
						self.dir = 1
						if self.Flag[0] == True and self.Flag[1] == False and self.Flag[2] == False and self.Flag[3] == False :
							self.player_image = pygame.transform.rotate(self.player, 0)
							self.player_init_y -= self.player_speed
					elif event.key == pygame.K_DOWN:
						self.Flag[1] = True
						self.dir = 2
						if self.Flag[0] == False and self.Flag[1] == True and self.Flag[2] == False and self.Flag[3] == False:
							self.player_image = pygame.transform.rotate(self.player, 180)
							self.player_init_y += self.player_speed	
					elif event.key == pygame.K_LEFT:
						self.Flag[2] = True
						self.dir = 3
						if self.Flag[0] == False and self.Flag[1] == False and self.Flag[2] == True and self.Flag[3] == False:
							self.player_image = pygame.transform.rotate(self.player, 90)
							self.player_init_x -= self.player_speed
					elif event.key == pygame.K_RIGHT:
						self.Flag[3] = True
						self.dir = 4
						if self.Flag[0] == False and self.Flag[1] == False and self.Flag[2] == False and self.Flag[3] == True:
							self.player_image = pygame.transform.rotate(self.player, 270)
							self.player_init_x += self.player_speed
					elif event.key == pygame.K_RETURN:
						self.player_x_pos = 0
						self.player_y_pos = 0 
			...
			self.player_x_pos += self.player_init_x
			self.player_y_pos += self.player_init_y 

			if self.Flag[0] == True:
				if self.collide > -1 and self.prev_dir == self.dir:
					self.stop[0] = 1
			if self.Flag[1] == True:
				if self.collide > -1 and self.prev_dir == self.dir:
					self.stop[1] = 1
			...
			if self.dir == 1:
				if self.collide > -1:
					if self.flag == False:
						...
						py = self.player_y_pos
						self.prev_dir = self.dir
						self.flag = True
					if self.stop[0] == True:
						self.player_y_pos = py
				elif self.collide <= -1:
					self.flag = False
			elif self.dir == 2:
				if self.collide > -1:
			...
			if self.player_x_pos < 0:
				self.player_x_pos = 0
			if self.player_x_pos > 454:
				self.player_x_pos = 454
			if self.player_y_pos < 0:
				self.player_y_pos = 0
			if self.player_y_pos > 390:
				self.player_y_pos = 390

			self.water_rect = self.water_image.get_rect()
			self.water_rect.left = 454
			self.water_rect.top = 390	

			if self.player_rect.colliderect(self.water_rect):
				back_ground_sound.stop()
				CheckBonusGame = True
				GameStart = False		
		

			if (self.endgame - self.startgame)/1000 > self.elapsedtime:
				back_ground_sound.stop()
				GameStart = False
				break
			
			screen.fill([0, 0, 0])
			screen.blit(self.water_image, [454, 390])
			screen.blit(self.player_image, [self.player_x_pos, self.player_y_pos])

			if (self.elapsedtime - (self.endgame - self.startgame)//1000) % 10 < 5:
				self.lv.draw([self.lv.TILE_EMPTY, self.lv.TILE_BRICK, self.lv.TILE_STEEL, self.lv.TILE_FROZE, self.lv.TILE_WATER])

			screen.blit(self.font.render("TIMER : " + str(self.elapsedtime - (self.endgame - self.startgame)//1000), True, pygame.Color('white')), [0, 0])
			screen.blit(self.font.render("RESET : Enter key" , True, pygame.Color('white')), [380, 0])
			pygame.display.flip()
			
	# 2번째 미니게임 함수입니다
	def MiniGame2(self):
		global total_score, screen, sprites, stage_round, CheckBonusGame, GameStart, quiz_count
		...
		while GameStart:
			...
			screen.fill([0, 0, 0])
			screen.blit(self.font.render("Totol Score : " + str(total_score), True, pygame.Color('white')), [100, 200])
			pygame.display.flip()
            		

	# 1번째 미니게임 함수입니다
	def MiniGame1(self): # Screen 480/416  416/16 = 26

		global screen, sprites, stage_round, CheckBonusGame, GameStart, back_ground_sound
		...
		self.player_init_y = 0
		self.player_speed = 6
		self.player_x_pos = 10
		self.player_y_pos = 192

		self.endgame = 0
		self.elapsedtime = 30

		so = SpawnOstacle(self.player_image, self.water_image)
		so1 = SpawnOstacle(self.player_image, self.water_image)
		so2 = SpawnOstacle(self.player_image, self.water_image)
		so3 = SpawnOstacle(self.player_image, self.water_image)
		so4 = SpawnOstacle(self.player_image, self.water_image)
		so5 = SpawnOstacle(self.player_image, self.water_image)
		so6 = SpawnOstacle(self.player_image, self.water_image)
		so7 = SpawnOstacle(self.player_image, self.water_image)
		so8 = SpawnOstacle(self.player_image, self.water_image)
		so9 = SpawnOstacle(self.player_image, self.water_image)
		so10 = SpawnOstacle(self.player_image, self.water_image)
		so11 = SpawnOstacle(self.player_image, self.water_image)
		so12 = SpawnOstacle(self.player_image, self.water_image)
		so13 = SpawnOstacle(self.player_image, self.water_image)

		self.obstacle_water_x1 = 0
		self.obstacle_water_x2 = 0
		self.obstacle_water_x3 = 0
		self.obstacle_water_x4 = 0
		self.obstacle_water_x5 = 0
		self.obstacle_water_x6 = 0
		self.obstacle_water_x7 = 0
		self.obstacle_water_x8 = 0
		self.obstacle_water_x9 = 0
		self.obstacle_water_x10 = 0
		self.obstacle_water_x11 = 0
		self.obstacle_water_x12 = 0
		self.obstacle_water_x13 = 0
		self.obstacle_water_x14 = 0
		...
		while GameStart:
			...
			self.obstacle_water_x1 = so.SpawnY(self.obstacle_water_x1, 0, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x2 = so1.SpawnY(self.obstacle_water_x2, 32, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x3 = so2.SpawnY(self.obstacle_water_x3, 64, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x4 = so3.SpawnY(self.obstacle_water_x4, 96, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x5 = so4.SpawnY(self.obstacle_water_x5, 128, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x6 = so5.SpawnY(self.obstacle_water_x6, 144, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x7 = so6.SpawnY(self.obstacle_water_x7, 176, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x8 = so7.SpawnY(self.obstacle_water_x8, 208, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x9 = so8.SpawnY(self.obstacle_water_x9, 240, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x10 = so9.SpawnY(self.obstacle_water_x10, 272, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x11 = so10.SpawnY(self.obstacle_water_x11, 304, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x12 = so11.SpawnY(self.obstacle_water_x12, 336, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x13 = so12.SpawnY(self.obstacle_water_x13, 368, self.player_x_pos, self.player_y_pos)
			self.obstacle_water_x14 = so13.SpawnY(self.obstacle_water_x14, 400, self.player_x_pos, self.player_y_pos)
			...
			screen.fill([0, 0, 0])
			screen.blit(self.player_image, [self.player_x_pos, self.player_y_pos])
			screen.blit(self.water_image, (self.obstacle_water_x1, 0))
			screen.blit(self.water_image, (self.obstacle_water_x2, 32))
			screen.blit(self.water_image, (self.obstacle_water_x3, 64))
			screen.blit(self.water_image, (self.obstacle_water_x4, 96))
			screen.blit(self.water_image, (self.obstacle_water_x5, 128))
			screen.blit(self.water_image, (self.obstacle_water_x6, 144))
			screen.blit(self.water_image, (self.obstacle_water_x7, 176))
			screen.blit(self.water_image, (self.obstacle_water_x8, 208))
			screen.blit(self.water_image, (self.obstacle_water_x9, 240))
			screen.blit(self.water_image, (self.obstacle_water_x10, 272))
			screen.blit(self.water_image, (self.obstacle_water_x11, 304))
			screen.blit(self.water_image, (self.obstacle_water_x12, 336))
			screen.blit(self.water_image, (self.obstacle_water_x13, 368))
			screen.blit(self.water_image, (self.obstacle_water_x14, 400))
...
# 1번째 미니게임에서 블록 설정하기위한 클래스입니다
class SpawnOstacle():
	def __init__(self,  water_image, player_image):
		global screen, sprites, stage_round, CheckBonusGame, GameStart, back_ground_sound
	...
	def SpawnY(self, obstacle_water_x_ ,obstacle_water_y_, player_init_x_, player_init_y_):

		global screen, sprites, stage_round, CheckBonusGame, GameStart, back_ground_sound
		
		self.obstacle_water_x_ = obstacle_water_x_
		self.obstacle_water_y_ = obstacle_water_y_

		self.player_init_x_ = player_init_x_
		self.player_init_y_ = player_init_y_

		
		if self.obstacle_water_x_ <= 0:
			self.water_speed = random.randint(3, 8)
			self.obstacle_water_x_ = 480

		self.obstacle_water_x_ -= self.water_speed

		self.player_rect = self.player_image.get_rect()
		self.player_rect.left = self.player_init_x_
		self.player_rect.top = self.player_init_y_

		self.water_rect = self.water_image.get_rect()
		self.water_rect.left = self.obstacle_water_x_
		self.water_rect.top = self.obstacle_water_y_	

		if self.player_rect.colliderect(self.water_rect):	
			back_ground_sound.stop()
			GameStart = False

		return self.obstacle_water_x_

# 3번째 미니게임의 퀴즈를 넣고 퀴즈를 내는 클래스입니다
class QuizData():
	def __init__(self):
		global total_score, screen, sprites, stage_round, CheckBonusGame, GameStart
		...
		self.quiz_data = [
    	{
        	"question": "On which game platform was \"Battle City\" first released?",
        	"choices": ["PlayStation", "Famicom"],
        	"answer": 1
    	},
    	{
        	"question": "What does the player control in \"Battle City\"?",
        	"choices": ["robot", "tank"],
        	"answer": 1
    	},
    	{
        	"question": "In what ways can a player destroy enemy tanks in the game?",
        	"choices": ["launch a missile", "Flight"],
        	"answer": 0
    	},
    	{
        	"question": "What goals should I achieve in the \"Battle City\" game?",
        	"choices": ["Destroy all enemy tanks", "Exploring the Map"],
        	"answer": 0
    	},
    	{
        	"question": "What purpose do you place walls in the \"Battle City\" game?",
        	"choices": ["To make a maze", "To prevent the enemy's tanks from moving"],
        	"answer": 1
    	},
    	{
        	"question": "What happens when a player's tank is destroyed in a game?",
        	"choices": ["Nothing Happens", "decrease in scores"],
        	"answer": 0
    	},
    	{
        	"question": "How does multiplayer mode work in \"Battle City\" games?",
        	"choices": ["Divide the screen and play", "play a card game"],
        	"answer": 0
    	},
    	{
        	"question": "What effect can items have in the game?",
        	"choices": ["End the game", "Strengthen the body"],
        	"answer": 1
    	},		
	        	{
        	"question": "What is the goal of the \"Battle City\" game?",
        	"choices": ["killing many enemies", "protecting a castle"],
        	"answer": 1
    	},
    	{
        	"question": "In what year was the \"Battle City\" game first released?",
        	"choices": ["1985", "2000"],
        	"answer": 0
    	}    	    
		]
	...
	def show_question(self):
		...
		global total_score, screen, sprites, stage_round, CheckBonusGame, GameStart, QuizFlag

		clock = pygame.time.Clock()
		sprites = pygame.transform.scale(pygame.image.load("images/sprites.gif"), [192, 224])
		self.player = sprites.subsurface(0*2, 0*2, 13*2, 13*2)
		self.wrong = sprites.subsurface(0, 80*2, 32*2, 32*2)
		self.player_image = pygame.transform.rotate(self.player, 270)

		pygame.init()
		self.CorrectSound = pygame.mixer.Sound("sounds/quizcorrect.mp3")
		self.WrongSound = pygame.mixer.Sound("sounds/wrongSelect.mp3")

		if QuizFlag == False:
			self.correct_quiz = [False, False, False, False, False]
			QuizFlag = True

		while True:
			self.current_question = random.randint(0, 9)
			if self._flag == False:
				self.current_question_prev = self.current_question
				self._flag = True
			if self.current_question == self.current_question_prev :
				continue
			else:
				self._flag = False
				break

		self.current_quiz = self.quiz_data[self.current_question]

		self.question_text = self.current_quiz["question"]
		self.choices = self.current_quiz["choices"]
		self.correct_answer = self.current_quiz["answer"]

		self.chosen_answer = -1

		self.startgame = pygame.time.get_ticks()
		self.sub_startgame = 0

		self.QuizStart = True

		while self.QuizStart:

			clock.tick(30)
			self.endgame = pygame.time.get_ticks()

			screen.fill([0, 0, 0])
			screen.blit(self.font.render("Quiz : " + str(quiz_count + 1), True, pygame.Color('white')), [2, 5])
			screen.blit(self.font1.render(str(self.question_text), True, pygame.Color('white')), [2, 50])
			screen.blit(self.font1.render(str(self.choices), True, pygame.Color('white')), [10, 100])
			screen.blit(self.font.render("TIMER : " + str(self.elapsedtime - (self.endgame - self.startgame)//1000), True, pygame.Color('white')), [2, 390])
			screen.blit(self.font.render("1 : ", True, pygame.Color('white')), [390, 200])
			screen.blit(self.font.render("2 : ", True, pygame.Color('white')), [390, 240])
			screen.blit(self.font.render("3 : ", True, pygame.Color('white')), [390, 280])
			screen.blit(self.font.render("4 : ", True, pygame.Color('white')), [390, 320])
			screen.blit(self.font.render("5 : ", True, pygame.Color('white')), [390, 360])
			if self.correct_quiz == [False, False, False, False, False]:
				screen.blit(self.wrong, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, False, False, False, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, True, False, False, False]:
				screen.blit(self.wrong, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, False, True, False, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, False, False, True, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, False, False, False, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, True, False, False, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, False, True, False, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, False, False, True, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, False, False, False, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, True, True, False, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, True, False, True, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, True, False, False, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, False, True, True, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, False, True, False, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, False, False, True, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, True, True, False, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, True, False, True, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, True, False, False, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, False, True, True, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, False, True, False, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, False, False, True, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, True, True, True, False]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [False, True, True, False, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, True, False, True, True]:
				screen.blit(self.wrong, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, False, True, True, True]:
				screen.blit(self.player_image, [410,170])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, True, True, True, False]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.wrong, [410,330])
			elif self.correct_quiz == [True, True, True, False, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.wrong, [410,290])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, True, False, True, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.wrong, [410,250])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, False, True, True, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.wrong, [410,210])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [False, True, True, True, True]:
				screen.blit(self.wrong, [410,170])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
				screen.blit(self.player_image, [427,350])
			elif self.correct_quiz == [True, True, True, True, True]:
				screen.blit(self.player_image, [427,190])
				screen.blit(self.player_image, [427,230])
				screen.blit(self.player_image, [427,270])
				screen.blit(self.player_image, [427,310])
			...
			if self.endgame - self.startgame > self.elapsedtime * 1000:
				self.sub_startgame = pygame.time.get_ticks()
				self.WrongSound.play()
				while True:
					self.sub_endgame = pygame.time.get_ticks()		
					screen.fill([0, 0, 0])
					screen.blit(self.font.render("Time over", True, pygame.Color('white')), [180, 180])
					pygame.display.flip()
					if self.sub_endgame - self.sub_startgame > 2000:
						break
				break
				...
				if event.type == pygame.KEYDOWN:
					if event.key == pygame.K_LEFT:
						...
						if self.correct_answer == self.chosen_answer:
							...
							self.sub_startgame = pygame.time.get_ticks()
							if quiz_count == 0:
								self.correct_quiz[0] = True
							elif quiz_count == 1:
								self.correct_quiz[1] = True
							elif quiz_count == 2:
								self.correct_quiz[2] = True			
							elif quiz_count == 3:
								self.correct_quiz[3] = True								
							elif quiz_count == 4:
								self.correct_quiz[4] = True
							while True:
								self.sub_endgame = pygame.time.get_ticks()		
								screen.fill([0, 0, 0])
								screen.blit(self.font.render("Total Score = " + str(total_score), True, pygame.Color('white')), [180, 180])
								pygame.display.flip()
								if self.sub_endgame - self.sub_startgame > 2000:
									break
							self.QuizStart = False
						elif self.correct_answer != self.chosen_answer:
							self.sub_startgame = pygame.time.get_ticks()
							self.WrongSound.play()
							while True:
								self.sub_endgame = pygame.time.get_ticks()		
								screen.fill([0, 0, 0])
								screen.blit(self.font.render("Wrong Selection", True, pygame.Color('white')), [170, 180])
								pygame.display.flip()
								if self.sub_endgame - self.sub_startgame > 2000:
									break
							self.QuizStart = False
...
if __name__ == "__main__":
	...
	pygame.init()
	back_ground_sound = pygame.mixer.Sound("sounds/background.ogg")
	stage_round = 0
	total_score = 0
	quiz_count = 0
	CheckBonusGame = False
	...
	bonus_game_stage = BonusGameStage()
	game = Game()
	castle = Castle()
	game.showMenu()